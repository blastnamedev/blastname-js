...
export const graphURIEndpoints: Record<string, string> = {
  ...
  168587773:
    "https://api.studio.thegraph.com/query/65930/blastname-testnet/version/latest",
  81457: "https://api.studio.thegraph.com/query/67907/blastname/version/latest",
};
...
export class ENS {
  ...
  constructor(options?: ENSOptions) {
    ...
    this.getContractAddress =
      options?.getContractAddress || _getContractAddress;
  }
  ...
  public getVouchers = this.generateFunction<FunctionTypes["getVouchers"]>(
    "initialGetters",
    ["gqlInstance"],
    "getVouchers"
  );

  public validateVoucher = this.generateRawFunction<
    FunctionTypes["validateVoucher"]
  >("validateVoucher", ["contracts"]);

  public validateReferrer = this.generateRawFunction<
    FunctionTypes["validateReferrer"]
  >("validateReferrer", ["contracts"]);

  // algo
  public mergedPool = {
    getPoolInfo: this.generateRawFunction<
      FunctionTypes["mergedPool"]["getPoolInfo"]
    >("mergedPool/getPoolInfo", ["contracts"]),
    getUserPoolAmount: this.generateRawFunction<
      FunctionTypes["mergedPool"]["getUserPoolAmount"]
    >("mergedPool/getUserPoolAmount", ["contracts"]),
    getUserPoolNfts: this.generateRawFunction<
      FunctionTypes["mergedPool"]["getUserPoolNfts"]
    >("mergedPool/getUserPoolNfts", ["contracts"]),
    getPendingToken: this.generateRawFunction<
      FunctionTypes["mergedPool"]["getPendingToken"]
    >("mergedPool/getPendingToken", ["contracts"]),
  };

  public vesting = {
    remainingLockedOf: this.generateRawFunction<
      FunctionTypes["vesting"]["remainingLockedOf"]
    >("vesting/remainingLockedOf", ["contracts"]),
    canUnlockAmount: this.generateRawFunction<
      FunctionTypes["vesting"]["canUnlockAmount"]
    >("vesting/canUnlockAmount", ["contracts"]),
  };

  public erc20 = {
    balanceOf: this.generateRawFunction<FunctionTypes["erc20"]["balanceOf"]>(
      "erc20/balanceOf",
      ["contracts"]
    ),
    allowance: this.generateRawFunction<FunctionTypes["erc20"]["allowance"]>(
      "erc20/allowance",
      ["contracts"]
    ),
    approve: this.generateWriteFunction<FunctionTypes["erc20"]["approve"]>(
      "erc20/approve",
      ["signer"]
    ),
    totalSupply: this.generateRawFunction<
      FunctionTypes["erc20"]["totalSupply"]
    >("erc20/totalSupply", ["contracts"]),
  };

  public nameWrapper = {
    isApprovedForAll: this.generateRawFunction<
      FunctionTypes["nameWrapper"]["isApprovedForAll"]
    >("nameWrapper/isApprovedForAll", ["contracts"]),
  };

  public uniswapV2Pair = {
    getReserves: this.generateRawFunction<
      FunctionTypes["uniswapV2Pair"]["getReserves"]
    >("uniswapV2Pair/getReserves", ["contracts"]),
  };

  public bank = {
    balanceOf: this.generateRawFunction<FunctionTypes["bank"]["balanceOf"]>(
      "bank/balanceOf",
      ["contracts"]
    ),
    canClaimReward: this.generateRawFunction<
      FunctionTypes["bank"]["canClaimReward"]
    >("bank/canClaimReward", ["contracts"]),
    canWithdraw: this.generateRawFunction<FunctionTypes["bank"]["canWithdraw"]>(
      "bank/canWithdraw",
      ["contracts"]
    ),
    earned: this.generateRawFunction<FunctionTypes["bank"]["earned"]>(
      "bank/earned",
      ["contracts"]
    ),
    epoch: this.generateRawFunction<FunctionTypes["bank"]["epoch"]>(
      "bank/epoch",
      ["contracts"]
    ),
    nextEpochPoint: this.generateRawFunction<
      FunctionTypes["bank"]["nextEpochPoint"]
    >("bank/nextEpochPoint", ["contracts"]),
    seatInfoOf: this.generateRawFunction<FunctionTypes["bank"]["seatInfoOf"]>(
      "bank/seatInfoOf",
      ["contracts"]
    ),
    withdrawLockupEpochs: this.generateRawFunction<
      FunctionTypes["bank"]["withdrawLockupEpochs"]
    >("bank/withdrawLockupEpochs", ["contracts"]),
    rewardLockupEpochs: this.generateRawFunction<
      FunctionTypes["bank"]["rewardLockupEpochs"]
    >("bank/rewardLockupEpochs", ["contracts"]),
  };

  public treasury = {
    epoch: this.generateRawFunction<FunctionTypes["treasury"]["epoch"]>(
      "treasury/epoch",
      ["contracts"]
    ),
    maxSupplyExpansionPercent: this.generateRawFunction<
      FunctionTypes["treasury"]["maxSupplyExpansionPercent"]
    >("treasury/maxSupplyExpansionPercent", ["contracts"]),
    seigniorageBankPercent: this.generateRawFunction<
      FunctionTypes["treasury"]["seigniorageBankPercent"]
    >("treasury/seigniorageBankPercent", ["contracts"]),
  };
}
