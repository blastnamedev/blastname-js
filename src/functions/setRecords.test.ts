...
describe("setRecords", () => {
  it("should return a transaction to the resolver and set successfully", async () => {
    const tx = await ensInstance.setRecords(`test123.${tldName}`, {
      records: {
        coinTypes: [
          {
            key: "ETC_LEGACY",
  ...
  describe("record types", () => {
    it("should be able to set and delete a cointype for a wrapped name", async () => {
      const tx = await ensInstance.setRecords(`wrapped.${tldName}`, {
        records: {
          coinTypes: [
            {
              key: "ETH",
      ...
      const tx2 = await ensInstance.setRecords(`wrapped.${tldName}`, {
        records: {
          coinTypes: [
            {
              key: "ETH",
