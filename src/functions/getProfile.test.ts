...
const checkRecords = (
  ...
  if (result) {
    ...
    expect(result.resolverAddress).toBe(deploymentAddresses.PublicResolver);
  }
};
...
describe("getProfile", () => {
  describe("with an address", () => {
    it("should return a profile object with no other args", async () => {
      ...
      expect(result).toBeDefined();
      if (result) {
        expect((result as any).name).toBe(`withprofile.${tldName}`);
  ...
  describe("with a name", () => {
    it("should return a profile object with no other args", async () => {
      const result = await ensInstance.getProfile(`withprofile.${tldName}`);
    ...
    it("should return a profile object for a specified resolver", async () => {
      ...
      const tx = await ensInstance.wrapName(`test123.${tldName}`, {
        wrappedOwner: accounts[1],
        addressOrIndex: 1,
      });
      await tx.wait();
      const result = await ensInstance.getProfile(`test123.${tldName}`, {
    ...
    it("should return the decoded name for a name with encoded labels", async () => {
      const result = await ensInstance.getProfile(
        `[9dd2c369a187b4e6b9c402f030e50743e619301ea62aa4c0737d4ef7e10a3d49].withsubnames.${tldName}`
      );
      expect(result).toBeDefined();
      expect(result?.decryptedName).toBe(`xyz.withsubnames.${tldName}`);
    });
    it("should return undefined for an unregistered name", async () => {
      const result = await ensInstance.getProfile(
        `test123123123cool.${tldName}`
      );
      expect(result).toBeUndefined();
    });
  ...
  describe("with invalid resolver", () => {
    ...
    it("should fail gracefully for a wrapped name with invalid resolver", async () => {
      ...
      expect(tx).toBeTruthy();
      await tx.wait();
      const result = await ensInstance.getProfile(`wrapped.${tldName}`);
      expect(result).toBeDefined();
      if (result) {
        expect(result.address).toBeUndefined();
        expect(Object.keys(result.records!).length).toBe(0);
  ...
  describe("errors", () => {
    ...
    it("should throw an ensjs error with no data", async () => {
      try {
        await ensInstance.getProfile(`withprofile.${tldName}`);
