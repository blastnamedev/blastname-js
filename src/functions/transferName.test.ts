...
describe("transferName", () => {
  ...
  it("should allow a transfer on the registrar", async () => {
    const tx = await ensInstance.transferName(`test123.${tldName}`, {
  ...
  it("should allow a transfer on the namewrapper", async () => {
    const tx = await ensInstance.transferName(`wrapped.${tldName}`, {
      newOwner: accounts[2],
    ...
    const result = await nameWrapper.ownerOf(namehash(`wrapped.${tldName}`));
    expect(result).toBe(accounts[2]);
  });
  it("should allow a transfer on the registry", async () => {
    const tx = await ensInstance.transferName(`test.withsubnames.${tldName}`, {
      newOwner: accounts[1],
      contract: "registry",
